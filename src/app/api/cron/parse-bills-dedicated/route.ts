...
export async function GET(request: Request) {
  ...
  try {
    ...
    paginate: while (true) {
      ...
      if (!listRes.ok) {
        return NextResponse.json({ error: 'Failed to list messages', parsed }, { status: 500 })
      }
      ...
      if (messages.length === 0) {
        console.log(
          `[parse-bills-dedicated] page=${pageNum} seen=0 already_in_db=0 processed=0`,
        )
        break
      }
      ...
      for (const { id: messageId } of messages) {
        ...
        // ── Property routing ──
        // Pre-stage: pick a "label-equivalent" candidate property from the
        // sender mapping or owner-name match. Then verifyBillRouting cross-
        // checks the PDF account number / address.
        ...
        if (mapping) {
          labelPropertyId = mapping.property_id
          preMatchSignal = 'learned_mapping'
        }

        // 2. Owner name (from subject or AI account_holder) — only if no sender mapping
        if (!labelPropertyId) {
        ...
        // 3. Verify (or override) using account number + address.
        // If we have NO candidate, treat label as empty and let the helper
        // fall through (it will return label_only with null; we'll then try
        // address-only resolution against the full property list below).
        ...
        if (labelPropertyId) {
          ...
          // No pre-match. Try account-number resolution directly; else fall
          // back to address fuzzy match against any property; else null.
        ...
        // Decide whether to auto-learn the sender mapping. Only do so on
        // verified routings — never on label_only (sender mapping was the
        // signal already; learning from itself adds nothing) and never on
        // mismatch.
        ...
        // Status: mismatch → flagged; else anomaly → flagged; else pending_review
      ...
      console.log(
        `[parse-bills-dedicated] page=${pageNum} seen=${pageSeen} already_in_db=${pageAlreadyInDb} processed=${pageProcessed}`,
      )
    ...
    return NextResponse.json({
      message: `Parsed ${parsed} bills from dedicated inbox`,
      parsed,
      skipped,
...
/**
 * Routing fallback when there's no pre-match candidate from sender
 * mapping or owner name. Tries account number + utility type, then
 * address; otherwise leaves `propertyId=null` so admin can route by hand.
 */
